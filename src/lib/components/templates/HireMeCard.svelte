...
<div class="flex items-center justify-center h-screen bg-gradient-to-br from-indigo-500 to-indigo-800">
    <div class="bg-white font-semibold text-center rounded-3xl border shadow-lg p-10 max-w-xs">
        ...
        <h1 
            ...
            contenteditable={editable} class="text-lg text-gray-700" > 
            {data.name || 'John Doe'} 
        </h1>
        <h3 
            id="bio"
            on:blur={(event) => fieldUpdate(event, data)}
            contenteditable={editable} class="text-sm text-gray-400" > 
            {data.bio || 'Creative Director'} 
        </h3>
        <p 
            id="description"
            on:blur={(event) => fieldUpdate(event, data)}
            contenteditable={editable} class="text-xs text-gray-400 mt-4"> {data.description || "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua."} 
        </p>
        {#if editable}
        ...
        {:else}
            <a href="{data['cta-url'] || '#'}"><button class="bg-indigo-600 px-8 py-2 mt-8 rounded-3xl text-gray-100 font-semibold uppercase tracking-wide">{data['cta-text'] || 'Hire Me'}</button></a>
        {/if}
    </div>
</div>